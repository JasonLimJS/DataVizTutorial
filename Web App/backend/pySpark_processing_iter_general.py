...
lagging_window= Window.partitionBy('ticker').orderBy(F.col('ticker').desc())
...
jdbcDF.filter(jdbcDF.ticker=='SABR').show(10)

jdbcDF= jdbcDF.withColumn('adjReturnPrice',F.log(jdbcDF.adjClosePrice/jdbcDF.adjClose_lag))
jdbcDF.select('ticker','adjClosePrice','adjClose_lag','adjReturnPrice').show(10)
jdbcDF.select('ticker','adjClosePrice','adjClose_lag','adjReturnPrice').filter(F.col('ticker')=='SABR').show(10)
